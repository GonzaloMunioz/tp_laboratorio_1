...
int IngresarEnteroValidado(char mensaje[], int min, int max, int* pNumeroIngresado)
{
	...
	*pNumeroIngresado = numeroIngresado;
	...
	{
		printf("El numero ingresado no corresponde con los parámetros. Debe estar en un rango entre %d hasta %d\n", min, max);
		IngresarEntero(mensaje, &numeroIngresado);
		*pNumeroIngresado = numeroIngresado;
	}
...
int ValidarAlfa(char *cadena)
{
	...
	if(cadena != NULL && strlen(cadena) > 0)
	{
		while((i < strlen(cadena) || cadena[i] != '\0') && flagNonAlphabetic == 0)
		{
			if(!isalpha(cadena[i]))
			{
				flagNonAlphabetic = 1;
