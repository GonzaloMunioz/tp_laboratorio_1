...
int registerEmployee(Employee* list, int len, int* id)
{
	...
	IngresarFlotante("Ingrese su salario: ", &employeeOne.salary);
	IngresarEntero("Ingrese el sector: ", &employeeOne.sector);
...
int modifyEmployee(Employee* list, int len)
{
	...
	if(indexEmployee != -1 && list != NULL)
	{
		...
		switch(option)
		{
			...
			case 3:
				IngresarFlotante("Ingrese su nuevo salario: ", &list[indexEmployee].salary);
				break;

			case 4:
				IngresarEntero("Ingrese su nuevo sector: ", &list[indexEmployee].sector);
				break;
		}
		retorno = 0;
	}
